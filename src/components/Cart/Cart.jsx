...
const Cart = () => {
  ...
  {cartData.map((each) => (
    <div
      key={each.id}
      className="cart-item-container flex justify-between align-items-center border rounded-xl p-3 m-3"
      style={{ width: "90%", maxWidth: "600px", margin: "auto" }}
    >
      {/* Left Section - Item Image and Details */}
      <div
        className="cart-item-info flex"
        onClick={() => navigation(`/item-details/${each.id}`)}
      ...
      >
        ...
        />
        <div className="item-details px-3">
          <div className="mb-3">
            <h1 className="font-semibold text-xl">{each.title}</h1>
            <h1 className="text-sm text-gray-500">{each.seller}</h1>
          </div>
      ...
      <div
        className="quantity-actions text-center my-auto"
        style={{ flex: 1 }}
      >
        <div className="quantity-control flex h-fit items-center gap-3 justify-center border-1 border-blue-500 w-fit rounded-md mx-auto">
          <button
            className="quantity-btn bg-green-500 p-2 text-white"
            onClick={() => handleAddQuantity(each.id)}
            key={`add-${each.id}`}
          >
            +
          </button>
          ...
          <button
            className="quantity-btn bg-green-500 p-2 text-white"
            onClick={() => handleRemoveQuantity(each.id)}
            key={`remove-${each.id}`}
          >
            -
          </button>
